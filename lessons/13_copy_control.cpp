...
static std::vector<int> foo_bar5()
{
	// Когда тип возвращаемого значения и локального объекта совпадает std::move возвращается АВТОМАТИЧЕСКИ
	std::vector<int> vec;

	// Когда критерии для пропуска копирования/перемещения (copy elision, в т.ч. RVO) 
	// соблюдены (кроме ограничений на параметры функции), и выражение обозначающее копируемый объект 
	// является lvalue, то выбор конструктора происходит так, как если бы это было rvalue.
	return vec;

	// На самом деле скорее всего произойдет не копирование,а  перемещение, если такое копирование
	// определено для объекта
	return std::move(vec); // не надо писать в вданном случае
}

// Таким образом, начиная с С++11 стандарт гарантирует, что если оптимизатор выключен, 
...
// При этом если выражение в return не является именем, 
// то автоматическое преобразование в rvalue не будет работать
T f1() {
  struct X { T t; } x;
...
// Если тип переменной отличается от типа возвращаемого значения, 
// то правила выше не действуют, и надо писать std::move(t):
struct Base {
  virtual ~Base() {}
};
struct Derived : Base {};
