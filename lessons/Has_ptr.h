...
public:
	...
	// присвоения копии, используя часть =delete вместо объявления их закрытыми (private)

	// Конструктор перемещения
	Has_ptr(Has_ptr&& h) noexcept: ps(h.ps), i(h.i) 
		{ std::cout << MODULE_NAME << "Move-Constructor called" << std::endl; h.ps = nullptr; }
...
#else
	// Оператор присвоения копии и обмена. При наличие конструктора перемещения этот оператор присвоения работает
	// как присвоение копий для l-value аргументов и как присвоение перемещения для r-value
	// (этот оператор присвоения является и оператором перемещения и оператором копий)

	// параметр - не ссылка, поэтому в зависимости от типа аргумента r-value или l-value будет вызываться конструктор
	// перемещения или копирования соответсвенно
