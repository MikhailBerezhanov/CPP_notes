...
{ 
	...
	const int * ptr_ci1 = &i;	// указатель на константу (по указателю нельзя поменять содержимое)
	// *ptr_ci1 = 10;			// ОШИБКА
	ptr_ci1 = nullptr;
	int const * ptr_ci2;		// указатель на константу 
	int * const cptr_i = &i;  	// константный указатель должен быть инициализирован при объявлении
	...
	// Ссылки. Не могут быть изменены после инициализации, не могут быть нулевыми 
	// в отличие от указателей. Неявно разыменовываются при обращении. Неконстантная ссылки
	// не может инициализироваться константой (4) или литералом ("hello").
	int value = 7;
	int *const ptr = &value;	// Эквивалентно (константный указатель)
	int &ref = value;			// Эквивалентно
	std::cout << *ptr << ref;	// Эквивалентно
